`freq_analysis.py` is a script that provides some utilities for frequence analysis, and a limited interface to do so. Here is a demonstration of its usage. (see `input.md` for more on how source input works).
